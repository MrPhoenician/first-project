...
`ps` - посмотреть запущенные процессы текущего пользователя
`bash` - запуск файла
`--help` - этот флаг выводит справку по флагам текущей команды
`man` - подробная справка по выбранной команде
...
С флагом `--graph` Git нарисует ветки с помощью «палочек» и «звёздочек»
...
`Git push —force` — это команда, которая позволяет отправить изменения в удалённый репозиторий без подтверждения. Эта команда отключает проверку коммитов и при необходимости выполняет перезапись истории.
...
Вернуться в главную ветку командой `git checkout main`
...
Суффикс навигации `~`. На практике чаще нужен либо текущий коммит (HEAD), либо следующий за ним (HEAD~1). Для ~1 есть специальное сокращение ~ (без числа).
`Git stash` - перемещает текущие изменения (так называемые local changes) в локальную директорию, которая выполняет роль специального хранилища, то есть скрывает эти изменения, сохраняя их отдельно, с опцией вернуть позже, когда это понадобится. Таким образом, файлы рабочей копии возвращаются к своему исходному состоянию. Внесенные изменения помещаются в стек, после чего их можно легко оттуда извлечь. Важно отметить, что в рабочей копии исчезнут все измененные файлы, — независимо от того, добавлены они в индекс или нет. После выполнения этой строки все несохраненные изменения будут сохранены, но не закоммичены. Эти отложенные участки будут сохранены в локальном репозитории и при выполнении команды git push не будут переданы на сервер. `git stash pop` - вернуть изменения.
```
$ git diff HEAD~ HEAD
...
Выполнить слияние: перейти в master и — `git merge (название_ветки)`
Удалить ветку после объединения — `git branch -D (название_ветки)`. У команды git branch -D есть более безопасный вариант с флагом `-d`. Он удалит ветку только если она была полностью объединена с другой — то есть если две ветки стали (или изначально были) частью одной истории.
Fast-forward слияние веток можно отключить флагом `--no-ff`. Например: `git merge --no-ff`. Если отключить слияние в режиме fast-forward, вместо «перемотки» ветки Git создаст в ней коммит слияния (merge commit).

## Забираем изменения из удалённого репозитория
Забрать изменения из удалённого репозитория — `git pull`
`Git fetch` — используется для скачивания изменений из удаленного или локального репозитория в свой локальный репозиторий. Польза git fetch в том, что эта команда привносит актуальность локальному репозиторию без изменения его дерева или текущей ветки.
